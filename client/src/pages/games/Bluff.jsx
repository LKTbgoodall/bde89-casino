      ...
      {b.state === "waiting" && (
        <div className="glass-card p-8 text-center border-t-4 border-indigo-500 flex flex-col items-center">
          ...
          <p className="text-zinc-400 mb-6">
            Inscris-toi sur la liste. Un joueur sera tiré au sort pour raconter
            son anecdote.
          </p>
          ...
          {inQueue ? (
            <div className="w-full">
              <div className="bg-indigo-500/20 text-indigo-300 py-3 rounded-xl mb-4 font-bold border border-indigo-500/30">
                ...
                {(b.queue?.findIndex((p) => p.id === player.id) ?? 0) + 1}/
                {b.queue?.length})
              </div>
              <button
                onClick={leaveQueue}
                className="text-rose-400 text-sm underline touch-manipulation"
              >
                Quitter la file
              </button>
            </div>
            ...
            <button
              onClick={joinQueue}
              className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-[0.98] text-lg touch-manipulation"
            >
              Rejoindre la partie
            </button>
          )}
          ...
          {b.queue?.length > 0 && (
            <div className="mt-6 w-full text-left">
              <h3 className="text-sm font-bold text-zinc-500 mb-2 uppercase">
                File d'attente :
              </h3>
      ...
      {b.state === "betting" && !playerActive && (
        <div className="glass-card p-6 border-t-4 border-amber-500">
          <div className="text-center mb-6">
            ...
            <span className="text-zinc-400 text-sm block mt-2">
              de raconter son anecdote !
            </span>
          </div>
